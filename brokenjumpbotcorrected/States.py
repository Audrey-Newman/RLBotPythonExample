...
def dependableController(agent, target_location, target_speed):
	...
	if agent.brokenJumpDetected: # Slow down instead of jumping when ball above car.
		ball_height = agent.ball.location.data[2]
		ball_distance = distance2D(agent.me, agent.ball)
		ball_on_wall = ballOnWall(agent)
	...
	if not agent.brokenJumpDetected:
		#dodging
		...
		if time_difference > 2.2 and distance2D(target_location,agent.me) > (velocity2D(agent.me)*2.5) and abs(angle) < 1.0 and agent.me.location.data[2] < 100:
			...
			if not agent.brokenJump:
				controller_state.jump = True
			controller_state.pitch = -1
			agent.heightAtJumpCmd = agent.me.location.data[2]
		...
		if not agent.brokenJumpDetected: # Once detected, can quit checking.
			if time_difference >= .3:
				if agent.me.location.data[2] <= agent.heightAtJumpCmd: # Height hasn't increased.
					agent.brokenJumpDetected = True
